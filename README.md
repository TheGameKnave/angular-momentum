...
### Reliability
- [ ] Measure how often sessions actually drop before treating expiry as normal. `persistSession` and `autoRefreshToken` are both on and Supabase refresh tokens are long-lived, so users being logged out "often" would be a bug, not a fact of life — candidates: the Tauri localStorage adapter losing data between launches, the 401 interceptor logging out on a refreshable error rather than retrying, or a storage clear. The 21.6.0 auto-open login prompt makes a dropped session *comfortable*; it does not make it *correct*, and it would happily paper over a real persistence bug.
...
### Test quality
- [ ] Sonar flags 10 `<dt>` findings (profile + notifications templates) under `Web:ItemTagNotWithinContainerTagCheck` — the markup is valid HTML (`<div>`-wrapped `dt/dd` groups inside `<dl>` are spec-legal); the rule predates the spec change. 2026-08: rule ignored in sonar-project.properties (with S2925 for performance.spec measurement windows); verify the open findings close after the next scan, otherwise mark them false-positive in the UI.
...
- [ ] E2E flake tail (retry-passers, ~1-2 per run). Root-caused and fixed 2026-07: storage-promotion seeding raced the IndexedDB load; indexeddb post-logout raced Supabase session clearing — the reloaded page booted still-authenticated and showed user-scoped data. 2026-08: storage-promotion decline raced the demo component's load-echo write, fixed in the component; two auth.spec tests clicked SSR DOM before hydration bound handlers, fixed with waitForAngular; notifications server-broadcast was NOT Supabase latency — io.emit is at-most-once and the page's socket can be mid-(re)connect when it fires (the suspected double-connect below widens the window), so the spec now re-sends until delivery (app-side fix: the double-connect cleanup in the Tauri item below); performance suite got the 60s budget the login suites already had; broadcasts also cross-contaminate parallel workers — every connected anonymous page stores the notification, making hasAnonymousData() true so the NEXT login in any worker gets blocked by an unexpected Import Local Data dialog — login flows now go through auth.helper's waitForLoginComplete, which skips it (kept as product behavior by choice, 2026-08).
- [ ] E2E: audit the remaining bare `goto → waitForSelector → interact` starts (profile.spec has ~12) for the hydration race fixed in auth.spec 2026-08 — SSR renders interactive elements before hydration binds their handlers; a click that lands in that window is silently inert. They haven't flaked because their first interaction trails several auto-retrying assertions, but that's incidental protection, not a guarantee.
